  ...
  return (
    <>
      ...
      {!isOpen && (
        ...
        >
          <Sparkles className="w-5 h-5 md:w-5 md:h-5 group-hover:animate-pulse" />
          <span className="font-medium text-xs md:text-sm hidden md:inline">Armá tu setup</span>
        </button>
      )}

      {/* Chat Panel */}
      {isOpen && (
        <div className="fixed bottom-6 right-6 z-[60] w-[400px] max-w-[calc(100vw-2rem)] h-[600px] max-h-[calc(100vh-3rem)] bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col overflow-hidden">
          {/* Header */}
