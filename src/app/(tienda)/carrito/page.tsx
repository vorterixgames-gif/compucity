...
export default function CartPage() {
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      applyCoupon(data.coupon as AppliedCoupon, data.discountAmount)
      setCouponCode('')
    } catch {
            ...
            {appliedCoupon && couponDiscount > 0 && (
              <div className="flex justify-between text-sm">
                <span className="text-green-600 flex items-center gap-1">
                  ...
                  <button onClick={removeCoupon} className="text-gray-400 hover:text-red-500 ml-1">
                    <X className="w-3 h-3" />
                  </button>
                </span>
            ...
            <div className="mb-4">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="CÓDIGO"
                  className="flex-1 border rounded-lg px-3 py-2 text-sm uppercase focus:outline-none focus:border-compucity-green"
                  onKeyDown={(e) => { if (e.key === 'Enter') handleApplyCoupon() }}
                />
                <button
                  onClick={handleApplyCoupon}
                  disabled={couponLoading || !couponCode.trim()}
                  className="px-4 py-2 bg-compucity-green text-white text-sm font-medium rounded-lg hover:bg-compucity-green-dark disabled:bg-gray-300 disabled:cursor-not-allowed transition"
                >
