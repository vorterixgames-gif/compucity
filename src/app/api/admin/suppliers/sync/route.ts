...
async function syncAirIntraBatch(supplier: any, batch: AirIntraBatchParams): Promise<SyncResult> {
  ...
  try {
    ...
    //
    // IMPORTANT: We do NOT skip pages 0..catalog_end_page for "probe" purposes.
    // Those pages contain existing products whose prices/stock need to be
    // updated on every sync. Skipping them would leave prices stale (which
    // was the bug that caused Air Intra price changes to not propagate).
    // The catalog_end_page marker is kept only as a diagnostic value.
      ...
      while (!pageSucceeded && retryCount <= MAX_RETRIES) {
        try {
          ...
          if (parseError) {
            ...
            if (!Array.isArray(parsedProducts) || parsedProducts.length === 0) {
              ...
              if (isBrokenPage) {
                ...
                if (newBrokenCount >= AIRINTRA_BROKEN_PAGE_THRESHOLD) {
                  ...
                  //
                  // This is END OF CATALOG, NOT a rate limit. Persist catalog_end_page
                  ...
                  result.ok = true
                  result.message = `END_OF_CATALOG: ${newBrokenCount} páginas consecutivas (desde la ${catalogEndPage + 1} hasta la ${page}) devolvieron respuestas vacías (notices PHP sin datos de productos). El catálogo de Air Intra termina en la página ${catalogEndPage}. Se procesaron ${totalFetched} productos en este lote. La sync se marcó como completa.`
                  result.total = totalFetched
                  result.created = created
                  result.updated = updated
                  result.skipped = skipped
            ...
            // catalog grew — clear catalog_end_page so the next END_OF_CATALOG
...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({
      ...
      catalogEndPage,                       // last known catalog end (-1 = unknown, diagnostic only)
