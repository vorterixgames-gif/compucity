/**
 * Direct Invid Computers supplier sync script
 * Fetches all products from Invid API and upserts them into the DB
...
const _entries = [
  ...
  ['EPSON','Epson'],['CANON','Canon'],['BROTHER','Brother'],['DELL','Dell'],
  ['ACER','Acer'],['HUAWEI','Huawei'],['XIAOMI','Xiaomi'],['TPLINK','TP-Link'],
  ['TENDA','Tenda'],['INTEL','Intel'],['SEAGATE','Seagate'],['TOSHIBA','Toshiba'],
  ['WESTERN','Western'],['WDC','WDC'],['CRUCIAL','Crucial'],['SANDISK','SanDisk'],
  ['ADATA','ADATA'],['KINGSTON','Kingston'],['PREDATOR','Predator'],['NITRO','Nitro'],
  ['THINKPAD','ThinkPad'],['VIVOBOK','VivoBook'],['ZENBOOK','ZenBook'],
  ['IDEACENTRE','IdeaCentre'],['PREDATOR','Predator'],['HP','HP'],
];
...
function getInvidSupplierCategory(product) {
...
async function main() {
  console.log('=== Invid Direct Sync ===\n');
  const startTime = Date.now();

  // 1. Get supplier credentials
  const supplierResult = await db.execute({ sql: 'SELECT * FROM suppliers WHERE id = ?', args: [INVID_SUPPLIER_ID] });
  const supplier = supplierResult.rows[0];
  if (!supplier) { console.error('ERROR: Supplier not found'); process.exit(1); }

  const apiUsername = supplier.apiUsername || '';
  const apiPassword = supplier.apiPassword || '';
  const markup = supplier.markup || 30;
  const baseUrl = supplier.apiBaseUrl || 'https://www.invidcomputers.com';

  if (!apiUsername || !apiPassword) { console.error('ERROR: Missing apiUsername or apiPassword'); process.exit(1); }

  console.log(`Supplier: ${supplier.name}, Username: ${apiUsername}, Markup: ${markup}%`);
  ...
  const supplierMappings = await buildSupplierMappingLookup(INVID_SUPPLIER_ID);
  console.log(`  Categories: ${Object.keys(slugToId).length}, Supplier mappings: ${Object.keys(supplierMappings).length}`);

  // 3. Pre-load ALL existing Invid product SKUs and ALL slugs into memory
  console.log('Loading existing products...');
  const existingResult = await db.execute({ sql: 'SELECT id, providerSku FROM products WHERE providerId = ?', args: [INVID_SUPPLIER_ID] });
  ...
  console.log(`  Found ${existingSkus.size} existing Invid products, ${allSlugs.size} total slugs in DB`);

  // 4. Authenticate with Invid API
  console.log('\nAuthenticating with Invid API...');
  let token;
  try {
    ...
    if (!authRes.ok) {
      ...
      console.error(`ERROR: Auth failed with HTTP ${authRes.status}: ${errText}`);
      process.exit(1);
    }

    const authData = await authRes.json();
    if (!authData.access_token) {
      console.error('ERROR: No access_token in auth response:', JSON.stringify(authData).substring(0, 300));
      process.exit(1);
    }
    ...
    console.error('ERROR: Auth request failed:', err.message);
    process.exit(1);
  }

  // 5. Fetch ALL products from Invid API (paginated)
  console.log('\nFetching all products from Invid API...');
  const allProducts = [];
  let offset = 0;
  const pageSize = 100;
  ...
  while (true) {
    ...
    try {
      ...
      const products = data.data || data.resultado || data.products || [];
      ...
      console.log(`    Got ${products.length} (total so far: ${allProducts.length})`);
  ...
  // 6. Process and build DB operations
  let created = 0;
  let updated = 0;
  let skipped = 0;
  let errors = 0;
  ...
  for (const product of allProducts) {
    try {
      ...
      const providerSku = String(product.ID || product.id || '');
      ...
      const productName = product.TITLE || product.title || product.DESCRIPTION || product.description || '';

      // Map stock status to numeric
      const stockStatus = (product.STOCK_STATUS || product.stock_status || '').toUpperCase().trim();
      let stock = 0;
      if (stockStatus === 'STOCK OK') stock = 10;
      else if (stockStatus === 'BAJO STOCK') stock = 3;
      else if (stockStatus === 'SIN STOCK') stock = 0;
      ...
      if (existingId) {
        ...
        updateStmts.push({
          ...
          args: [costPrice, sellingPrice, stock, supplierCategory, categoryId, new Date().toISOString(), existingId],
        });
        updated++;
      } else {
        // INSERT
        ...
        const images = product.IMAGE_URL || product.image_url ? JSON.stringify([product.IMAGE_URL || product.image_url]) : '[]';
        const specs = {};
        if (product.BRAND || product.brand) specs['Marca'] = product.BRAND || product.brand;
        if (product.PART_NUMBER || product.part_number) specs['Part Number'] = product.PART_NUMBER || product.part_number;
        if (product.RUBRO || product.rubro) specs['Rubro'] = product.RUBRO || product.rubro;
        if (product.CATEGORIA || product.categoria) specs['Categoría'] = product.CATEGORIA || product.categoria;
        if (product.GRUPO || product.grupo) specs['Grupo'] = product.GRUPO || product.grupo;
        if (product.FAMILY || product.family) specs['Familia'] = product.FAMILY || product.family;

        const finalPrice = parseFloat(product.FINAL_PRICE || product.final_price || '0');
        ...
        insertStmts.push({
          ...
          args: [
            newId, formattedName, slug, product.DESCRIPTION || product.description || product.LONG_DESCRIPTION || '',
            sellingPrice, finalPrice > 0 ? finalPrice * (1 + markup / 100) : null,
            costPrice, providerSku, stock, 1, 0, images, JSON.stringify(specs),
            INVID_SUPPLIER_ID, providerSku, categoryId, supplierCategory,
          ],
        });
        created++;
      }
  ...
  // 7. Execute DB operations in batches
  ...
  await db.execute({ sql: 'UPDATE suppliers SET lastSyncAt = ?, updatedAt = ? WHERE id = ?', args: [syncNow, syncNow, INVID_SUPPLIER_ID] });

  // 9. Count total Invid products in DB
  const countResult = await db.execute({ sql: 'SELECT COUNT(*) as total FROM products WHERE providerId = ?', args: [INVID_SUPPLIER_ID] });
  const totalInDb = Number(countResult.rows[0].total);
  ...
  console.log(`Total Invid products in DB: ${totalInDb}`);
